...
FitSmQ <- function(Q, Qreg, method = "monoH.FC"){
 ...
 pA1 <- ggplot(aes(days, lFlow), data = Q) + geom_line()
 pA2 <- pA1 + xlab("Days") + ylab("log(Flow)") + ggtitle("log-scale")
 pA3 <- pA2 + geom_point(aes(days, lpQ), data = Qreg, colour = "purple")

 pB1 <- ggplot(aes(days, Flow), data = Q) + geom_line()
 pB2 <- pB1 + xlab("Days") + ylab("Flow (CM/s)") + ggtitle("natural scale")
 pB3 <- pB2 + geom_point(aes(days, pQ), data = Qreg, colour = "purple")

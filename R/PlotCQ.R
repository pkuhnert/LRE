...
PlotCQ <- function(Conc, Q){
  ...
  p1 <- ggplot(aes_string('Date', 'lConc'), data = Conc) + geom_line() + ylab("log(Concentration)")
  p2 <- ggplot(aes_string('Date', 'lpQ'), data = Conc) + geom_line() + ylab("log(Flow)")
  ...
  p3 <- ggplot(aes_string('Date', 'lpQ'), data = Qsub) + geom_line(colour = "blue")
